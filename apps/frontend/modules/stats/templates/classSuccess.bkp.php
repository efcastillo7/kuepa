...
<table class="table table-hover">
	...
	<tbody>
		...
		<tr>
			...
			<td><?php echo gmdate("H:i:s", LogService::getInstance()->getTotalTimeByRoute($student->getId(), array('course_id' => $course->getId()))) ?></td>
			<td><?php echo utcToLocalDate($student->getLastAccess(), "dd/M/yyyy H:mm") ?></td>
			<td>Lecciones Completadas</td>

			<!-- por unidad -->
			<?php foreach ($course->getChapters() as $chapter): ?>
			<td><?php echo gmdate("H:i:s", LogService::getInstance()->getTotalTimeByRoute($student->getId(), array('course_id' => $course->getId(), 'chapter_id' => $chapter->getId()))) ?></td>
			<td>Progreso</td>
			<?php endforeach ?>
			<!-- fin por unidad -->
		</tr>
